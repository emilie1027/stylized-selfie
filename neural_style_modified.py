...
def parse_args():
  ...
  parser.add_argument('--region_control', action='store_true',
    help='Transfer the style to masked regions.')

  parser.add_argument('--content_regions', nargs='+', type=str,
    default=None,
    help='Filenames of masks that could segment content image and turn them into different styles')

  parser.add_argument('--style_regions', nargs='+', type=str,
    default=None,
...
def stylize(content_img, style_imgs, init_img, frame=None):
  with tf.device(args.device), tf.Session() as sess:
    ...
    elif args.region_control:
      L_style = sum_region_style_losses(sess, net, style_imgs)
    else:
...
def write_image_output(output_img, content_img, style_imgs, init_img):
  ...
  cv2.normalize(
      output_img, output_img, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
...
def get_content_image(content_img):
  ...
  if h > w and h > mx:
    ...
    img = cv2.resize(img, dsize=(int(w), mx), interpolation=cv2.INTER_LINEAR)
  if w > mx:
    h = (float(mx) / float(w)) * h
    img = cv2.resize(img, dsize=(mx, int(h)), interpolation=cv2.INTER_LINEAR)
  img = preprocess(img, vgg19_mean)
  return img
...
def get_style_images(content_img):
  ...
  for style_fn in args.style_imgs:
    ...
    check_image(img, path)
    img = img.astype(np.float32)
    img = cv2.resize(img, dsize=(cw, ch), interpolation=cv2.INTER_LINEAR)
    img = preprocess(img, vgg19_mean)
